...
function H5up_done(f,h){var k=".php?album="+h5u_albSel.value;redirURL=0<js_vars.user_id||1==js_vars.guest_edit?js_vars.site_url+"/editpics"+k+"&newer_than="+js_vars.timestamp:js_vars.site_url+"/thumbnails"+k;f&&fetch("notifyupload.php",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({album:h5u_albSel.value})}).then(l=>l.text()).then(l=>{console.log("Success:",l);"1"==js_vars.autoedit&&0===h&&(window.location=redirURL)}).catch(l=>{console.error("Error:",l)});$id("gotoedit").style.display=
"table-row"}
...
0;q=!1;t.style.display="none";v.innerHTML=0;r||M()}};$ae(f,"DOMContentLoaded",function(){var b=$id("upload_field"),a=$id("dropArea");f.File&&f.FileList?(v=$id("qcount"),p=$id("qstop"),t=$id("qgocan"),b&&($ae(b,"click",l),$ae(b,"change",K)),b=new XMLHttpRequest,b.upload&&($ae(a,"dragover",C),$ae(a,"dragleave",C),$ae(a,"drop",K),a.style.display="block",u=$id("totprogress"),I=$id("fprogress")),h5u_albSel=document.getElementsByName("h5u_album")[0]):($id("h5upldrow").style.display="none",$id("navailrow").style.display=
"table-row")})})(window,"uniload.php","autorient flistitl title caption keywords user1 user2 user3 user4".split(" "));function showAllowedExts(){alert(js_vars.h5uM.extallow+js_vars.allowed_file_types.join(", "))}function shide_titlrow(f){var h=$id("titlrow");f.checked?h.style.display="none":h.style.display="table-row"};