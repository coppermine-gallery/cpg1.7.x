...
EOT;
    ...
    if (GALLERY_ADMIN_MODE) {
        $CAT_LIST[] = [0, $lang_modifyalb_php['no_cat']];
    }

    //add user catergory
    $CAT_LIST[] = [(FIRST_USER_CAT + USER_ID), $lang_modifyalb_php['my_gal']];
...
EOT;
    ...
    $thumbs = [$cpg_nopic_data['thumb']];

    $img_list = [0 => $lang_modifyalb_php['last_uploaded'], -1 => $lang_modifyalb_php['random_image']];
...
function form_visibility($text, $name)
{
    ...
    if (GALLERY_ADMIN_MODE) {
        ...
        $options = [$lang_modifyalb_php['public_alb'], FIRST_USER_CAT + USER_ID => $lang_modifyalb_php['me_only']];
...
function form_moderator($text, $name)
{
    ...
    $options = [$lang_modifyalb_php['admins_only']];
