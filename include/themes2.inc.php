...
// to get an idea of what to copy into your custom theme.      //
...
{
	$menu[] = [$href_lnk,$href_title,$href_tgt,$block_id,$spacer,$href_attrib];
}
/******************************************************************************
...
EOT;
	...
	$params = ['{BUTTONS}' => assemble_template_buttons($template_sys_menu_button,$sys_menu_buttons)];
...
if (!defined('THEME_HAS_NO_SUB_MENU_BUTTONS')) {
	...
	$params = ['{BUTTONS}' => assemble_template_buttons($template_sub_menu_button,$sub_menu_buttons)];
...
<!-- BEGIN catrow_noalb -->
	<tr>
		<td class="catrow_noalb" colspan="3"><table><tr><td align="left">{CAT_THUMB}</td><td align="left"><span class="catlink">{CAT_TITLE}</span>{CAT_DESC}</td></tr></table></td>
	</tr>
<!-- END catrow_noalb -->
<!-- BEGIN catrow -->
	<div>
		<div class="cat-af-cnt" align="center">{PIC_COUNT} {PICTURES}</div>
		<div class="cat-af-cnt" align="center">{ALB_COUNT} {ALBUMS}</div>
		<div class="catrow" align="left">
			<!-- <div>{CAT_THUMB}</div> -->
			<div><span class="catlink">{CAT_TITLE}</span><p>{CAT_DESC}</p></div>
		</div>
	</div>
	<div>
		<div class="tableb tableb_alternate" colspan="3">{CAT_ALBUMS}</div>
	</div>
<!-- END catrow -->
<!-- BEGIN footer -->
	<tr>
	...
	<div width="{COL_WIDTH}%" valign="top">
		<div width="100%">
			...
			<div width="100%" valign="top" class="tableb tableb_alternate">
				<div class="thumbnails" style="background-color:transparent"><img src="images/spacer.gif" width="1" height="1" class="image" style="border:0;margin-top:1px;margin-bottom:0" alt="" /></div>
			</div>
		</div>
	</div>
<!-- END empty_cell -->
...
$template_film_strip = <<<EOT
	<tr>
		<td valign="top" class="filmstrip_background" style="background-image: url({TILE1});"><img src="{TILE1}" alt="" /></td>
	</tr>
	<tr>
	...
	<tr>
		<td valign="top" class="filmstrip_background" style="background-image: url({TILE2});"><img src="{TILE2}" alt="" /></td>
	</tr>
<!-- BEGIN thumb_cell -->
	<td class="thumb" >
...
$template_user_list_info_box = <<<EOT
	<table cellspacing="1" cellpadding="0" width="100%" class="user_thumb_infobox">
		<tr>
...
<!-- BEGIN comment_captcha -->
						<tr>
							...
							<td class="tableb tableb_alternate" colspan="2">
								...
								<img src="captcha.php" align="middle" alt="" />
							</td>
						</tr>
<!-- END comment_captcha -->
					</table>
			...
			<table cellspacing="0" cellpadding="10" bgcolor="#ffffff">
				<tr>
					...
					<td valign="top" width="300">
						<div align="right"><img src="{URL_PREFIX}images/stamp.gif" alt="" /></div>
						<br />
	...
	<tr>
		<td bgcolor="#000000">
			<table cellspacing="0" cellpadding="10" bgcolor="#ffffff">
				<tr>
					<td valign="top">
						<a href="{PIC_TGT}"><img src="{PIC_URL}" alt="" /></a><br />
					</td>
...
if (!function_exists('theme_css_head')) {  //{THEMES}
/******************************************************************************
** Section <<<theme_css_head>>> - START
******************************************************************************/
// Function for the CSS inside the <head>-section
function theme_css_head()
{
	global $CSS, $LINEBREAK;

	$return = '';

	// Check if we have any CSS includes
	if (!empty($CSS['includes'])) {
//		$JS['includes'] = CPGPluginAPI::filter('javascript_includes',$JS['includes']);
		// Include all the files which were set using css_include() function
		foreach ($CSS['includes'] as $css_file) {
	...
	if (!empty($CSS['rules'])) {
		// Join the rules
		$css_rules = implode($LINEBREAK, $CSS['rules']);
		$return .= <<<EOT
	<style>
	{$css_rules}
	</style>
...
function theme_create_tabs($items, $curr_page, $total_pages, $template)
{
	...
	if (array_key_exists('page_link',$template)) {
		...
		$template['nav_tab'] = strtr($template['nav_tab'], ['{LINK}' => $template['page_link']]);
		$template['inactive_tab'] = strtr($template['inactive_tab'], ['{LINK}' => $template['page_link']]);
	}
...
function theme_admin_mode_menu()
{
	...
	if ($admin_menu == '') {
		...
		if (GALLERY_ADMIN_MODE) {
			...
			$param = [
				'{ALBMGR_TITLE}' => $lang_user_admin_menu['albmgr_title'],
...
function theme_display_cat_list($breadcrumb, &$cat_data, $statistics)
{
	...
	foreach ($cat_data as $category) {
		...
		$stack[] = "</div attr=\"{$category['cat_level']}\">";
...
function theme_display_album_list(&$alb_list, $nbAlb, $cat, $page, $total_pages)
{
	...
	$theme_alb_list_tab_tmpl['left_text'] = strtr($theme_alb_list_tab_tmpl['left_text'], ['{LEFT_TEXT}' => $lang_album_list['album_on_page']]);
	$theme_alb_list_tab_tmpl['page_link'] = strtr($theme_alb_list_tab_tmpl['page_link'], ['{LINK}' => 'index.php?cat=' . $cat . '&amp;page=%d']);
	...
	if ($STATS_IN_ALB_LIST) {
		$params = ['{STATISTICS}' => $statistics, '{COLUMNS}' => $columns];
...
//	$params = ['{COL_WIDTH}' => $column_width, '{SPACER}' => $thumb_cell_width];
...
//		$spacer = template_extract_block($template_thumbnail_view, 'spacer');
	...
	$album_types = ['albums' => ['lastalb']];
	$album_types = CPGPluginAPI::filter('theme_thumbnails_album_types', $album_types);

	$theme_thumb_tab_tmpl = $template_tab_display;

	if ($mode == 'thumb') {
		$theme_thumb_tab_tmpl['left_text'] = strtr($theme_thumb_tab_tmpl['left_text'], ['{LEFT_TEXT}' => in_array($aid, $album_types['albums']) ? $lang_album_list['album_on_page'] : $lang_thumb_view['pic_on_page']]);
		$theme_thumb_tab_tmpl['page_link'] = strtr($theme_thumb_tab_tmpl['page_link'], ['{LINK}' => 'thumbnails.php?album=' . $aid . $cat_link . $date_link . $uid_link . '&amp;page=%d']);
	} else {
		$theme_thumb_tab_tmpl['left_text'] = strtr($theme_thumb_tab_tmpl['left_text'], ['{LEFT_TEXT}' => $lang_thumb_view['user_on_page']]);
		$theme_thumb_tab_tmpl['page_link'] = strtr($theme_thumb_tab_tmpl['page_link'], ['{LINK}' => 'index.php?cat=' . $cat . '&amp;page=%d']);
	}
...
function theme_no_img_to_display($album_name)
{
	...
	$params = ['{TEXT}' => $lang_errors['no_img_to_display']];
...
function theme_html_picture()
{
	...
	if ($mime_content['content']=='image') {
		...
		if ($image_size['width'] > $image_size['height']) {
			$img_class = 'image-l';
			$img_style = 'style="max-width:'.$image_size['width'].'px"';	//'width="100%" max-width="'.$CURRENT_PIC_DATA['pwidth'].'" height="auto"';
		} else {
			$img_class = 'image-p';
			$img_style = 'style="max-height:'.$image_size['height'].'px"';	//'width="100%" max-width="'.$CURRENT_PIC_DATA['pwidth'].'" height="auto"';
		}
		...
		if ($CURRENT_PIC_DATA['mode'] != 'fullsize') {
			...
			if ($CONFIG['transparent_overlay'] == 1) {
				$pic_html = '<table cellpadding="0" cellspacing="0"><tr><td background=\"' . $picture_url . "\" width=\"{$image_size['width']}\" height=\"{$image_size['height']}\" class=\"image\">";
				...
				$pic_html .= "<img src=\"images/image.gif?id=".floor(rand()*1000+rand())."\" width=\"{$image_size['width']}\" height=\"{$image_size['height']}\" alt=\"{$pic_title}\" title=\"{$pic_title_fs}\" /><br />";
				...
				$pic_title_fs = $lang_display_image_php['view_fs'] . $LINEBREAK . '==============' . $LINEBREAK . $pic_title;
				$pic_html .= "<img src=\"" . $picture_url . "\" {$img_style} class=\"{$img_class}\" alt=\"{$pic_title}\" title=\"{$pic_title_fs}\" /><br />";
				$pic_html .= $pic_html_href_close;
				//PLUGIN FILTER
			...
			if ($CONFIG['transparent_overlay'] == 1) {
				...
				$pic_html .= "<img src=\"images/image.gif?id=".floor(rand()*1000+rand())."\" width={$CURRENT_PIC_DATA['pwidth']} height={$CURRENT_PIC_DATA['pheight']} alt=\"{$pic_title}\" title=\"{$pic_title}\" /><br />" . $LINEBREAK;
				$pic_html .= "</td></tr></table>";
				//PLUGIN FILTER
				$pic_html = CPGPluginAPI::filter('html_image_overlay', $pic_html);
			} else {
				$pic_html = "<img src=\"" . $picture_url . "\" {$image_size['geom']} class=\"image\" alt=\"{$pic_title}\" title=\"{$pic_title}\" /><br />" . $LINEBREAK;
				//PLUGIN FILTER
	...
	if ($num) {
		...
		while ($row = $result->fetchAssoc()) { // while-loop start
			...
			if (USER_IS_ADMIN) {
				...
				if ($row['approval'] == 'NO') {
					$pending_approval = '<img src="' . $location . 'images/icons/comment_disapprove_disabled.png" alt="" width="16" height="16" class="icon" /><a href="reviewcom.php?pos=-{PID}&amp;msg_id={MSG_ID}&amp;form_token={FORM_TOKEN}&amp;timestamp={TIMESTAMP}&amp;what=approve" title="' . $lang_display_comments['approve'] . '"><img src="' . $location . 'images/icons/comment_approve.png" alt="" width="16" height="16" class="icon" /></a>';
				} else {
					$pending_approval = '<a href="reviewcom.php?pos=-{PID}&amp;msg_id={MSG_ID}&amp;form_token={FORM_TOKEN}&amp;timestamp={TIMESTAMP}&amp;what=disapprove" title="' . $lang_display_comments['disapprove'] . '"><img src="' . $location . 'images/icons/comment_disapprove.png" alt="" width="16" height="16" class="icon" /></a><img src="' . $location . 'images/icons/comment_approve_disabled.png" alt="" width="16" height="16" class="icon" />';
				}
				...
				if ($row['approval'] == 'NO') { // the comment is not approved - start
					if ($user_can_edit) { // the comment comes from the current visitor, display it with a warning that it needs admin approval
						$pending_approval = '<img src="' . $location . 'images/icons/comment_approval.png" alt="" width="16" height="16" title="' . $lang_display_comments['pending_approval'] . '" class="icon" />';
			...
			list($row['ip_detail']) = CPGPluginAPI::filter('ip_information', ['', $row['msg_hdr_ip']]);
			$ip = $row['msg_hdr_ip']. $row['ip_detail'];
			if ($row['msg_hdr_ip'] != $row['msg_raw_ip']) {
				list($row['ip_detail']) = CPGPluginAPI::filter('ip_information', ['', $row['msg_raw_ip']]);
			...
			$params = [
				'{MSG_AUTHOR_LNK}' => $profile_lnk,
				'{MSG_AUTHOR}' => $row['msg_author'],
				'{MSG_ID}' => $row['msg_id'],
				'{PID}' => $row['pid'],
				'{EDIT_TITLE}' => &$lang_display_comments['edit_title'],
				'{DELETE_TITLE}' => &$lang_display_comments['delete_title'],
				'{DELETE_ICON}' => '<img src="' . $location . 'images/icons/delete.png" alt="" width="16" height="16" class="icon" />',
				'{EDIT_ICON}' => '<img src="' . $location . 'images/icons/edit.png" alt="" width="16" height="16" class="icon" />',
				...
				'{REPORT_COMMENT_ICON}' => '<img src="' . $location . 'images/icons/report.png" alt="" width="16" height="16" class="icon" />',
				'{WIDTH}' => $CONFIG['picture_table_width'] == "100%" ? $CONFIG['main_table_width'] : $CONFIG['picture_table_width'],
				'{FORM_TOKEN}' => $form_token,
				'{TIMESTAMP}' => $timestamp
				];
...
function theme_slideshow($start_img,$title)
{
	...
	$params = [
		...
		'{IMAGE}' => '<img id="showImage" src="' . $start_img . '" class="image" alt="" /><br />',
		'{ADMIN_MENU}' => '',
...
function theme_display_fullsize_pic()
{
	...
	if ($superCage->get->keyExists('picfile')) {
		...
		$imagedata = ['name' => $picfile, 'path' => path2url($picname), 'geometry' => $imagesize[3]];
	} elseif ($pid) {
		...
		if (is_image($row['filename'])) {
			...
			$imagedata = ['name' => $row['filename'], 'path' => $pic_url, 'geometry' => $geom];
		} else {
...
function theme_vanity()
{
	...
	return template_eval($template_vanity, []);
}
/******************************************************************************
	...
	)
{
	...
	$return .= '<table cellspacing="0" cellpadding="0" width="'.$maxBarSizeInPixels.'">';
	$return .= '<tr>';
	...
	$return .= '">';
	$return .= '<img src="images/spacer.gif" width="1" height="16" alt="" align="left" />';
	if ($textShadowColor != '') {
