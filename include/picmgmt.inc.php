...
function add_picture($aid, $filepath, $filename, $position = 0, $title = '', $caption = '', $keywords = '', $user1 = '', $user2 = '', $user3 = '', $user4 = '', $category = 0, $raw_ip = '', $hdr_ip = '', $iwidth = 0, $iheight = 0)
{
    ...
    if (!is_known_filetype($image)) {
        return ['error' => $lang_db_input_php['err_invalid_fext'] . ' ' . $CONFIG['allowed_file_extensions'], 'halt_upload' => 0];
        ...
        if (max($imagesize[0], $imagesize[1]) > $CONFIG['max_upl_width_height']) {
            if ((USER_IS_ADMIN && $CONFIG['auto_resize'] == 1) || (!USER_IS_ADMIN && $CONFIG['auto_resize'] > 0)) {
                ...
                return ['error' => $msg, 'halt_upload' => 1];
            }
        }
    ...
    if (!GALLERY_ADMIN_MODE && $USER_DATA['group_quota'] && $category == FIRST_USER_CAT + USER_ID) {
        ...
        if ((($total_space_used + $total_filesize)>>10) > $USER_DATA['group_quota'] ) {
            ...
            $msg = $lang_errors['quota_exceeded'] . '<br />&nbsp;<br />' . strtr($lang_errors['quota_exceeded_details'], ['[quota]' => ($USER_DATA['group_quota']), '[space]' => ($total_space_used >> 10)]);
            return ['error' => $msg, 'halt_upload' => 1];
        }
    }
    ...
    $CURRENT_PIC_DATA['mime'] = $imagesize['mime'];
    $CURRENT_PIC_DATA['ftype'] = cpg_get_type($filename)['content'];
    ...
    $CURRENT_PIC_DATA['ctime'] = time();
    $CURRENT_PIC_DATA['filepath'] = cpg_db_escape_string($CURRENT_PIC_DATA['filepath']);
    $CURRENT_PIC_DATA['filename'] = cpg_db_escape_string($CURRENT_PIC_DATA['filename']);
	...
	$datakeys = join(', ', array_keys($CURRENT_PIC_DATA));
	$datavalues = '\'' . join('\', \'', array_values($CURRENT_PIC_DATA)) . '\'';

    $query = "INSERT INTO {$CONFIG['TABLE_PICTURES']} ({$datakeys}) VALUES ({$datavalues})";
