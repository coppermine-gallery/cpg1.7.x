...
EOT;
        if (empty($result) || strlen($result['body']) < 200) {
            print <<< EOT
                      <iframe src="//coppermine-gallery.net/cpg17x_news.htm" align="left" frameborder="0" scrolling="auto" marginheight="0" marginwidth="0" width="100%" height="100" name="coppermine_news" id="coppermine_news" class="textinput">
...
function css_include($filename, $inline = false)
{
    global $CSS;
    ...
    // If we need to show the css inline then return the required html
    if ($inline) {
    	return '<link href="'.$filename.'" rel="stylesheet" type="text/css"/>';
    } else {
        // Else add the file to js includes array which will later be used in head section
        $CSS['includes'][] = $filename;
    }
} // function css_include


function set_css_rules($rules)
{
    global $CSS;
